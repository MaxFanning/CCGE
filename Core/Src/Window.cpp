...
void Window::Initialize(HINSTANCE instance, LPCSTR appName, uint32_t  width, uint32_t height)
{
	...
	// Every Windows Window requires at least one window objects. Three things are invovled:
	// 1)	Register a window class.
	// 2)	Create a window objects.
	// 3)	Retrieve and dispatch messaged for this window
	...
	VERIFY(RegisterClassExA(&wcex), "[Core::Window] Failed to register windows class.");
	...
	const int top = (screenWidth - winHeight) >> 1;
